...
/**
 * 
 * @param {[string, string, string]} targets 
 */
function findTriple(targets) {
    const [first, second, third] = targets;
    const potentials = [... new Set(first.split(''))];
    for (const potential of potentials) {
        if (second.includes(potential) && third.includes(potential)) return potential;
    }
    return '';
}
